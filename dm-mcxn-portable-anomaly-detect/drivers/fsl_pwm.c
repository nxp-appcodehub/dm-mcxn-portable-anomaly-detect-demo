...
static inline uint16_t dutyCycleToReloadValue(float dutyCyclePercent)
{
 ...
 */
static void PWM_SetPeriodRegister(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwmSignal, pwm_mode_t mode, uint16_t pulseCnt)
{
    uint16_t modulo = 0;
    ...
    switch (mode)
    {
	    ...
	    if ((pwmSignal == kPWM_PwmA) || (pwmSignal == kPWM_PwmB)) /* Work Around - Jira Ticket:
	                                                                 https://jira.sw.nxp.com/browse/MCUX-69597*/
	    {
	    ...
	    else if (pwmSignal == kPWM_PwmX) /* Work Around - Jira Ticket:
	                                        https://jira.sw.nxp.com/browse/MCUX-69597*/
	    {
	    	/* Setup the PWM period for a signed center aligned signal */
	            modulo = (pulseCnt >> 1U);
	            ...
	            /* VAL0 Register controls the duty cycle for PWM_X */
	    ...
	    //base->SM[1].INIT=0xD23B; //the modulo is 256
        ...
        case kPWM_SignedCenterAligned:
            ...
            else if (pwmSignal == kPWM_PwmB)
            {
                ...
                base->SM[subModule].VAL5 = (pwmHighPulse / 2U);
                                                 ...
                                                 https://jira.sw.nxp.com/browse/MCUX-69597*/
            {
            	uint16_t pwmLowPulse = pulseCnt - pwmHighPulse;
            	base->SM[subModule].VAL0 = PWM_GetComplementU16(pulseCnt / 2) + pwmLowPulse;
            }
            break;
        case kPWM_CenterAligned:
                      ...
                      uint32_t srcClock_Hz)
{
    ...
    for (i = 0; i < numOfChnls; i++)
    {
        if (chnlParams[i].pwmChannel == kPWM_PwmX)
        {
        	...
        	/* Work Around - Jira Ticket:  https://jira.sw.nxp.com/browse/MCUX-69597 */
        }
    }
    ...
    for (i = 0; i < numOfChnls; i++)
    {
        ...
        if (i == 0U)
        {
            ...
            PWM_SetPeriodRegister(base, subModule, chnlParams->pwmChannel, mode, pulseCnt);
        }
        ...
        else if (chnlParams->pwmChannel == kPWM_PwmB) /* Work Around - Jira Ticket:
                                                         https://jira.sw.nxp.com/browse/MCUX-69597*/
        {
        ...
        else if (chnlParams->pwmChannel == kPWM_PwmX) /* Work Around - Jira Ticket:
                                                         https://jira.sw.nxp.com/browse/MCUX-69597*/
        {
        ...
        else
	{
        ...
        switch (chnlParams->pwmChannel)
        {
            ...
            case kPWM_PwmB:
                ...
                break;
            case kPWM_PwmX: /* Work Around - Jira Ticket:
                               https://jira.sw.nxp.com/browse/MCUX-69597*/
                            ...
                            float dutyCyclePercent)
{
    ...
    /*assert(pwmSignal != kPWM_PwmX);*/   /* Work Around - Jira Ticket:
                                             https://jira.sw.nxp.com/browse/MCUX-69597*/
    ...
    PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwmSignal, pwm_mode_t currPwmMode, uint16_t dutyCycle)
{
    ...
    else if (kPWM_PwmX == pwmSignal) /* Work Around - Jira Ticket:
                                        https://jira.sw.nxp.com/browse/MCUX-69597*/
   {
	PWM_SetDutycycleRegister(base, subModule, kPWM_PwmX, currPwmMode, pulseCnt, pwmHighPulse);
   }
                                     ...
                                     uint16_t dutyCycle)
{
    ...
    /* Update register about period */
    PWM_SetPeriodRegister(base, subModule, pwmSignal, currPwmMode, pulseCnt);
